...
namespace NzbDrone.Core.Test.Datastore
{
    ...
    public class MarrDataLazyLoadingFixture : DbTest
    {
        ...
        public void Setup()
        {
            ...
            {
                releases.Add(
                    Builder<Release>.CreateNew()
                    .With(v => v.Id = 0)
                    .With(v => v.ReleaseGroupId = album.Id)
            ...
            var tracks = Builder<Track>.CreateListOfSize(10)
                ...
                .With(v => v.ReleaseId = releases[0].Id)
                .BuildListOfNew();
        ...
        public void should_lazy_load_profile_if_not_joined()
        {
            ...
            var tracks = DataMapper.Query<Track>()
                .Join<Track, Release>(JoinType.Inner, v => v.Release, (l, r) => l.ReleaseId == r.Id)
                .Join<Release, Album>(JoinType.Inner, v => v.ReleaseGroup, (l, r) => l.ReleaseGroupId == r.Id)
                .Join<Album, Artist>(JoinType.Inner, v => v.Artist, (l, r) => l.ArtistMetadataId == r.ArtistMetadataId)
                .ToList();

            foreach (var track in tracks)
            {
                Assert.IsTrue(track.Release.IsLoaded);
                Assert.IsTrue(track.Release.Value.ReleaseGroup.IsLoaded);
                Assert.IsTrue(track.Release.Value.ReleaseGroup.Value.Artist.IsLoaded);
                Assert.IsNotNull(track.Release.Value.ReleaseGroup.Value.Artist.Value);
                Assert.IsFalse(track.Release.Value.ReleaseGroup.Value.Artist.Value.Profile.IsLoaded);
                Assert.IsFalse(track.Release.Value.ReleaseGroup.Value.Artist.Value.LanguageProfile.IsLoaded);
            }
        }
        ...
        public void should_explicit_load_profile_if_joined()
        {
            ...
            foreach (var track in tracks)
            {
                ...
                Assert.IsTrue(track.Release.Value.ReleaseGroup.Value.Artist.IsLoaded);
                Assert.IsNotNull(track.Release.Value.ReleaseGroup.Value.Artist.Value);
                Assert.IsTrue(track.Release.Value.ReleaseGroup.Value.Artist.Value.Profile.IsLoaded);
                Assert.IsFalse(track.Release.Value.ReleaseGroup.Value.Artist.Value.LanguageProfile.IsLoaded);
            }
        }
        ...
        public void should_explicit_load_languageprofile_if_joined()
        {
            ...
            foreach (var track in tracks)
            {
                ...
                Assert.IsNotNull(track.Release.Value.ReleaseGroup.Value.Artist.Value);
                Assert.IsFalse(track.Release.Value.ReleaseGroup.Value.Artist.Value.Profile.IsLoaded);
                Assert.IsTrue(track.Release.Value.ReleaseGroup.Value.Artist.Value.LanguageProfile.IsLoaded);
            }
        }
