...
namespace NzbDrone.Core.Music
{
    ...
    public class RefreshTrackService : IRefreshTrackService
    {
        ...
        public void RefreshTrackInfo(Album rg)
        {
            _logger.Info("Starting track info refresh for: {0}", rg);
            var successCount = 0;
            var failCount = 0;

            foreach (var release in rg.Releases.Value)
            {
                ...
                foreach (var track in OrderTracks(dupeFreeRemoteTracks))
                {
                    try
                    {
                        ...
                        trackToUpdate.ReleaseId = release.Id;
                    ...
                    {
                        _logger.Fatal(e, "An error has occurred while updating track info for release group {0}. {1}", rg, track);
                        failCount++;
                    }
                }
            ...
            if (failCount != 0)
            {
                _logger.Info("Finished track refresh for release group: {0}. Successful: {1} - Failed: {2} ",
                    rg.Title, successCount, failCount);
            }
            else
            {
                _logger.Info("Finished track refresh for rg: {0}.", rg);
            }
        }
        ...
        {
            return tracks.OrderBy(e => e.ReleaseId).ThenBy(e => e.TrackNumber);
        }
    }
}
