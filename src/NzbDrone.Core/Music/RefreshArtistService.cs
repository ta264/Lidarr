...
namespace NzbDrone.Core.Music
{
    public class RefreshArtistService : IExecute<RefreshArtistCommand>
    {
        ...
        private void RefreshArtistInfo(Artist artist, bool forceAlbumRefresh)
        {
            ...
            var remoteAlbums = artistInfo.ReleaseGroups.Value.DistinctBy(m => new { m.ForeignReleaseGroupId, m.ReleaseDate }).ToList();
            ...
            foreach (var album in remoteAlbums)
            {
                ...
                var albumToRefresh = existingAlbums.FirstOrDefault(s => s.ForeignReleaseGroupId == album.ForeignReleaseGroupId);
